...
if [ -z "$NM_BIN" ] && [ -z "$OBJDUMP_BIN" ]; then
    die "Neither nm nor objdump found. Cannot inspect binary artifact."
fi
...
if [ -n "$NM_BIN" ]; then
    ...
    # Run nm with demangling; ignore errors if specific sub-object has no symbols
...
if [ "$FAILURES" -gt 0 ]; then
    die "Binary inspection failed with $FAILURES violation(s)."
fi
