...
- **`.cargo/hide-libm-shadow.map`** — a linker version script listing the
  ...
  ever end up defined in `NAM-Plug` binaries again — from any dependency, now
...
- **`build.rs`** — applies the version script (plus `-Wl,--undefined-version`,
  ...
  `cargo:rustc-link-arg`, scoped to `NAM-Plug`'s own link targets only.
...
- Any time the toolchain (`rustc`/`compiler_builtins`) or the linker
  (`ld`/`lld`/`bfd`) changes, run the `NAM-Plug` test suite (`utils/tests-quick.sh`) as a sanity check before trusting a release build.
